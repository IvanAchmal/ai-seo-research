# Research on AI-powered SEO content production

> **Start here:** [Playbook / SOP — AI-Powered SEO Content Production](playbook.md)

The main deliverable is the playbook above — a cited, phase-by-phase operating guide built from 10 expert sources, including where experts disagree, what was rejected, original ideas, and known weaknesses.

Below is the curated research collection that informed it: 10 pieces of content (YouTube videos and LinkedIn posts), all published in 2026, covering the intersection of AI and SEO strategy.

## **THE OVERALL LOGIC OF THE COLLECTION**

The collection intentionally spans three dimensions of the topic:
 
  1. Tactical/practical AI SEO — how to actually use AI tools to rank
  2. Scalable content production — programmatic and system-level approaches
  3. Strategic/big-picture shifts — how AI is reshaping search itself
     (GEO, AI Overviews, Preferred Sources)
 
## **EXPERT-BY-EXPERT RATIONALE**
 
- [**Julian Goldie**](ai-powered-seo-content-production/youtube-transcripts/julian-goldie.md)  
  Why selected: A top practitioner known for actionable, results-driven SEO. His video directly addresses using Claude AI for ranking — perfectly on-topic for AI-powered SEO content production. He runs Goldie Agency, has ranked thousands of sites, and has taught over 50,000 students on Udemy.
 
- [**Matt Diggity**](ai-powered-seo-content-production/youtube-transcripts/matt-diggity.md)  
  Why selected: Known for data-driven, experiment-based SEO. He runs actual tests and shares the data — no guessing. His video on Reddit and AI Search directly addresses how user-generated content is influencing AI search results.
 
- [**Nathan Gotch**](ai-powered-seo-content-production/youtube-transcripts/nathan-gotch.md)  
  Why selected: CEO of Rankability and Head AI SEO Coach at Rankability Academy, recognized across Semrush and Ahrefs for data-driven SEO training. His video on "AI SEO skills that matter most in 2026" is a direct skills-mapping resource for the topic.
 
- [**Jake Ward**](ai-powered-seo-content-production/linkedin-posts/jake-ward.md)  
  Why selected: A young entrepreneur who founded an SEO agency generating $20M annually for clients, and built Byword — an AI content generation tool. His LinkedIn piece on building 13,000+ pages in 3 hours using programmatic SEO is the most directly relevant to AI-powered content production at scale.
 
- [**Kevin Indig**](ai-powered-seo-content-production/linkedin-posts/kevin-indig.md)  
  Why selected: A strategic growth advisor and confirmed speaker at major marketing conferences in 2026 covering AI-first search landscapes. His content on building durable AI SEO strategies brings long-term thinking — balancing the tactical pieces from others.
 
- [**Ross Hudgens**](ai-powered-seo-content-production/linkedin-posts/ross-hudgens.md)  
  Why selected: Founder of Siege Media, a respected content-focused SEO agency. His post on "The Category Leader's Trap" offers a contrarian, strategic angle — useful for research to avoid blind spots in dominant approaches.
 
- [**Koray Tuğberk GÜBÜR**](ai-powered-seo-content-production/linkedin-posts/koray-tugberk.md)  
  Why selected: A leading voice on topical authority and semantic SEO — concepts central to how AI evaluates content depth and credibility. His work on extension domains speaks to advanced content architecture strategies.
 
- [**Neil Patel**](ai-powered-seo-content-production/youtube-transcripts/neil-patel.md)  
  Why selected: One of the most widely followed digital marketers globally. His 2026 video on how marketing is changing provides broad context on the shift AI is causing — useful as a foundational framing piece in any research.
 
- [**Ross Simmonds**](ai-powered-seo-content-production/linkedin-posts/ross-simmonds.md)  
  Why selected: Known as a content distribution and strategy expert. His post on Google's Preferred Sources feature is highly relevant, as this directly impacts which AI-assisted content gets surfaced and cited by Google.
 
- [**Caleb Ulku**](ai-powered-seo-content-production/youtube-transcripts/caleb-ulku.md)  
  Why selected: Included for his sharp, timely commentary on Google's CEO-level decisions and their impact on SEO — capturing how platform-level changes affect AI content strategies.

## **SUMMARY OF COLLECTION LOGIC**

The collection is well-curated for its purpose. It balances hands-on practitioners (Goldie, Diggity, Ward) with strategists (Indig, Hudgens, Simmonds) and technical specialists (Gotch, Koray), while including broad-reach voices (Neil Patel, Caleb Ulku) to anchor the research in mainstream relevance. All content is from 2026, ensuring the references reflect the current AI search landscape rather than outdated tactics.